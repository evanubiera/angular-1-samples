	...
	return {
		...
		controller: ['$scope', '$element', '$attrs', '$timeout', function($scope, $element, $attrs, $timeout) {
			...
			$scope.start = $element[0].start = function() {				
				$scope.startTime = Date.create($scope.startTimeAttr);
				$scope.endTime = Date.create($scope.endTimeAttr);
			...
			//show alert confirm message button after 8 minutes
			$scope.$watch('minutes', function(newValue, oldValue) {
				if (newValue >= 8 || $scope.hours > 0 || $scope.days > 0) {
					$("#" + $scope.orderId).show();
				}
			});
